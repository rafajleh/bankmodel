...
export const QrcodeDisplayData = () => {
  ...
  const fetchData = async (nid, mobile) => {
    try {
      const response = await fetch(`http://localhost:5000/api/qrcode/getuserdatabyqr?nid=${encodeURIComponent(nid)}&mobile=${encodeURIComponent(mobile)}`); // Replace with your API endpoint
      if (!response.ok) {
  ...
  const handelAddPayment = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/qrcode/addpayment?uid=${data._id}`); // Replace with your API endpoint
      if (!response.ok) {
        throw new Error('Network response was not ok.');
      }
      // console.log(await response.json())
